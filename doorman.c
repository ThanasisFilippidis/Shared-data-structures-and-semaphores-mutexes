...
int main(int argc, char const *argv[])
{
	...
	sharedStruct *sh;
	sh=(sharedStruct *)shmat(shmid, (void*)0, 0); /* Attach the segment */
	...
	for (i = 0; i < sh->numTables; ++i)		//finding the biggest table capacity in the restaurant
	{
	...
	while(1){
		...
		sleep(serviceTime);		//doorman needs some time to serve the customers in each way
		int value=0,position=-1,servicedBar=0,tableId=-1;
		sem_wait(&sh->tableMutex);
		if (existsFreeTable(tables, sh->numTables) && sh->barCurr)		//if there are free tables and people in bar
		{																//then try to serve them first, they have priority
			for (i = sh->barStart; i < sh->barEnd; ++i)
			{
				...
				if ((tableId=existsFreeTableOf(bar[i],tables, sh->numTables)) >= 0)	//find the first group that fits in the available tables
				{
					value=bar[i];
					position=i;
					servicedBar=1;
			...
			if (value <= 2 && position >= 0)		//depending on the group that found fitting changes the bar and the tables statuses
			{
		...
		if (sh->queueCurr && !servicedBar)	//if there is people out of the restaurant and in this loop didnt served someone from bar
		{									//serve someone from out
			sem_post(&sh->doorman);
			sem_wait(&sh->companySizeReported);
			if (sh->doorCompanySize > sh->biggestTableCap)		//if there is no big enough table for this company
			{
	...
	err=shmdt((void *) sh); /* Detach segment */
